...
function Post(props) {
  ...
  function renderPostList() {
    return post.data ? (
      <div className="card">
        ...
        <p className="py-6">{post.data.content}</p>
        <hr />
        <small
          style={{
            display: 'block',
        ...
        >
          Paylaşan:  {post.data.user.firstName || ''} {post.data.user.lastName || ''}
          <br />
          Paylaşma tarihi: {post.data.createdAt}
        </small>
      </div>
    ) : (
        <div className="container">
          <div class="card">Paylaşım bulunamadı</div>
        </div>
      );
  }
  ...
  return (
    <Layout
      ...
      origin={origin}>
      <div class="flex max-w-screen-lg" style={{ margin: "0 auto" }}>
